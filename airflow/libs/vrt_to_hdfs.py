    ...
    with Kerberos(krb_config['principal'], krb_config['keytab']):
        with vertica_python.connect(**conn_info) as conn:
            with conn.cursor() as cur:
                cur.execute(f'''EXPORT TO PARQUET(directory='webhdfs:///data/vertica/ODS_LEAD_GEN_TST/KW_WORD_ENTITY_FRAME_TST3', compression='snappy') AS SELECT * FROM ODS_LEAD_GEN.KW_WORD_ENTITY_FRAME;''')
                data = cur.fetchall()
                print(data)